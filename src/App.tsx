  ...
  return (
    <div className="min-h-screen bg-[#f9f9f7] text-[#1a1c1b] flex flex-col selection:bg-[#fed65b] selection:text-black">
      ...
      <main className="flex-1 w-full pt-28">
        ...
        {currentScreen === 'account' && <CustomerAccountScreen />}
        {currentScreen === 'admin' && <AdminPortal />}
        {currentScreen === 'logistics' && <LogisticsPortal />}
      ...
      {/* Authentication & RBAC Modal (Login, Registration, OTP, Admin Clearance) */}
      <AuthModal />

      {/* Toast Notification Pill */}
      {toastMessage && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-black/90 text-white backdrop-blur-md px-4 py-2.5 rounded-full font-body-sm text-body-sm shadow-xl flex items-center gap-2 animate-in fade-in slide-in-from-top-2">
